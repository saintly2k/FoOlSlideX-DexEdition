...
<div class="col-span-8 grid grid-cols-12 gap-2">
    ...
    <form method="POST" name="addGroup" class="col-span-12 w-full" enctype="multipart/form-data">
        <div class="form-group mb-3">
            ...
            <input required type="text" minlength="10" maxlength="100" id="name" name="name" class="w-full" placeholder="Name of Group" value="<?= isset($_POST["name"]) ? clean(mysqli_real_escape_string($conn, $_POST["name"])) : "" ?>">
            <p class="text-gray-500 leading-none">This is the Name of the Group. It needs to be between 10 and 100 Characters. It may contain special Characters.</p>
        </div>
        <div class="form-group mb-3">
            <label for="banner" class="text-xl pl-5">Banner <span class="text-gray-600">(Optional)</span></label>
            <input class="block w-full text-sm text-gray-900 bg-gray-50 border border-gray-300 cursor-pointer" id="banner" name="banner" type="file">
        ...
        <div class="form-group mb-3">
            <label for="upload" class="text-xl pl-5">Upload Permissions <span class="text-gray-600">(Optional)</span></label>
            <input <?= $userlevel["mod"] != 1 ? "disabled" : "" ?> type="text" id="upload" name="upload" class="w-full" placeholder="Array of User IDs seperated by comma" value="<?= isset($_POST["upload"]) ? clean(mysqli_real_escape_string($conn, $_POST["upload"])) : "" ?>">
        ...
        <div class="form-group mb-3">
            <label for="edit" class="text-xl pl-5">Edit Permissions <span class="text-gray-600">(Optional)</span></label>
            <input <?= $userlevel["mod"] != 1 ? "disabled" : "" ?> type="text" id="edit" name="edit" class="w-full" placeholder="Array of User IDs seperated by comma" value="<?= isset($_POST["edit"]) ? clean(mysqli_real_escape_string($conn, $_POST["edit"])) : "" ?>">
        ...
        <?php if ($userlevel["mod"] != 1) { ?>
            <div class="form-group mb-3">
                <p class="text-center text-gray-500">If you are unable to edit the Upload and Edit Permissions, it means you are missing the necessary rights. You can change this later on after the group has been approved and you appointed as leader.</p>
            </div>
        <?php } ?>
        <?php if ($userlevel["mod"] == 1) { ?>
            <div class="form-group mb-3">
                <label for="mod_status" class="text-xl pl-5">Mod: Group Status <span class="text-gray-600">(Required)</span></label>
                <select required class="w-full" id="mod_status" name="mod_status">
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" id="mod_owner" name="mod_owner" class="w-full" placeholder="Singe User ID of user" value="<?= isset($_POST["mod_owner"]) ? clean(mysqli_real_escape_string($conn, $_POST["mod_owner"])) : "" ?>">
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" id="mod_redirect" name="mod_redirect" class="w-full" placeholder="Singe Group ID" value="<?= isset($_POST["mod_redirect"]) ? clean(mysqli_real_escape_string($conn, $_POST["mod_redirect"])) : "" ?>">
